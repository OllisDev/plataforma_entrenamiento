...
function listarSesionesBloque() {
    ...
    fetch(url, {
        ...
        .then((res) => res.json())
        .then((data) => {
            crearTabla(data["sesion-plan"]);
            setTimeout(listarSesionesBloque, 60000); // refresca la tabla cada 60 segundos sin que el usuario tenga que recargar la pagina
        })
        .catch((error) => {
            console.log(error);
...
function crearTabla(sesionBloque) {
